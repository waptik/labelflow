...
type ParserLabel = Pick<DbLabel, "type" | "geometry" | "labelClassId"> &
...
const runIog = async (
  ...
  const centerPoint = [(width - x) / 2, (height - y) / 2];
  const iogLabel = await fetchIogServer({
    id: uuid(),
    imageUrl: dataUrl,
    x,
    y,
...
const runInference = async (
  ...
  const iogLabels = useAutoPolygon
...
const runAiAssistant = async (
  _: any,
  {
    data: { imageId, assistantId, useAutoPolygon },
  }: MutationRunAiAssistantArgs,
  ctx: Context
): Promise<RunAiAssistantOutput> => {
  const assistant = getAiAssistant(assistantId);
